...
startNo = 1
endNo   = 8
subPath <- "http://internationalnewsstation.tw/?event_area=cross-strait"
alldata = data.frame()
for( pid in startNo:endNo )
{
  ...
  title   <- xpathSApply(xmldoc, "//div[@class='block-space fleft']//h4", xmlValue)
  title   <- gsub("\n", "", title)
  title   <- gsub("\t", "", title)
  path    <- xpathSApply(xmldoc, "//div[@class='block-title']//a[@class='more']//@href")
  cite    <- xpathSApply(xmldoc, "//div[@class='block-title']//cite")
  article <- xpathSApply(xmldoc, "//div[@class='block-title']//p")
...
suburlPath <- "http://internationalnewsstation.tw/?event_area=cross-strait"
